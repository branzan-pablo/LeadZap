    ...
    return (
      <div className="mt-4 space-y-3">
        {Array.from({ length: 3 }).map((_, i) => (
          <div
            key={i}
            className="max-w-[70%] self-start rounded-2xl rounded-tl-sm border border-zinc-200 bg-zinc-50 px-3 py-2"
          >
  ...
  if (messages.length === 0) {
    return (
      <div className="mt-8 flex flex-col items-center text-center">
        <MessageCircleIcon className="size-10 text-zinc-300" />
        <p className="mt-3 text-sm font-medium text-zinc-600">
          Nenhuma mensagem do WhatsApp
        </p>
        <p className="mt-1 text-xs text-zinc-400">
          Mensagens recebidas aparecerão aqui automaticamente.
        </p>
      </div>
    )
  }
